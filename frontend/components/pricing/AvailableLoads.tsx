...
function statusBadge(status: LoadStatus) {
  const map: Record<LoadStatus, string> = {
    ...
    'Quoting': 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300',
    'Offers Received': 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
    'Rate Selected': 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
...
function modeBadge(mode: TransportMode) {
  const map: Record<TransportMode, string> = {
    FCL: 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/30 dark:text-indigo-300', LCL: 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-300',
    FTL: 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300', LTL: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300',
    AIR: 'bg-sky-100 text-sky-800 dark:bg-sky-900/30 dark:text-sky-300',       PARCEL: 'bg-pink-100 text-pink-800 dark:bg-pink-900/30 dark:text-pink-300',
    RAIL: 'bg-teal-100 text-teal-800 dark:bg-teal-900/30 dark:text-teal-300',    BULK: 'bg-stone-100 text-stone-800 dark:bg-stone-900/30 dark:text-stone-300',
    SPECIAL: 'bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300',
  };
  return map[mode] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200';
}
    ...
    <div className="bg-white dark:bg-[#1E1E1E] rounded-lg shadow">
      <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-gray-900 dark:text-gray-100">{initial ? 'Edit Load' : 'New Load'}</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">{initial ? `Load ${initial.loadNumber}` : 'Create a new pricing request'}</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6">
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Load Number</label>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Qty</label>
            <input type="number" value={quantity} onChange={e => setQuantity(Number(e.target.value))} min={1}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 dark:text-gray-100" />
          </div>
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Volume (CBM)</label>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Cargo Nature *</label>
            <input value={cargoNature} onChange={e => setCargoNature(e.target.value)} required placeholder="Electronics, Machinery…"
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 dark:text-gray-100" />
          </div>
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Incoterm</label>
          ...
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Required Date *</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Sales Notes</label>
            <textarea value={salesNotes} onChange={e => setSalesNotes(e.target.value)} rows={2}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 dark:text-gray-100" />
          </div>
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Pricing Notes</label>
...
function AddQuoteForm({ load, onSaved, onCancel }: AddQuoteFormProps) {
  ...
  return (
    <div className="bg-gray-50 dark:bg-[#262626] border border-gray-200 dark:border-gray-700 rounded-lg p-5 mt-2">
      <h4 className="text-gray-900 dark:text-gray-100 mb-4">Add Supplier Quote — {load.loadNumber}</h4>
      {error && <div className="mb-3 px-3 py-2 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded text-sm text-red-700 dark:text-red-300">{error}</div>}
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Carrier / Line</label>
        ...
        <div className="grid grid-cols-4 gap-4">
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Offered Rate</label>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Validity Date</label>
            <input type="date" value={validityDate} onChange={e => setValidityDate(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 dark:text-gray-100" />
          </div>
          <div>
            <label className="block text-sm text-gray-700 dark:text-gray-300 mb-1">Equipment Available</label>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl text-gray-900 dark:text-gray-100">Available Loads</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage pricing requests and supplier quotes</p>
        </div>
      ...
      <div className="text-sm text-gray-500 dark:text-gray-400">Showing {filtered.length} of {loads.length} loads</div>
      ...
      {loading ? (
        ...
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-6 text-center text-red-600 dark:text-red-300">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                {filtered.length === 0 ? (
                  <tr><td colSpan={10} className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No loads found. Click "New Load" to create one.</td></tr>
                ) : filtered.map(l => (
                  <Fragment key={l.id}>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                      <td className="px-4 py-3 text-sm text-blue-600">{l.loadNumber}</td>
                      <td className="px-4 py-3">
                        <div className="text-sm text-gray-900 dark:text-gray-100">{l.clientName || '—'}</div>
                        {l.salesPerson && <div className="text-xs text-gray-500 dark:text-gray-400">{l.salesPerson}</div>}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{l.equipmentType || '—'}</td>
                      <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">
                        {l.quantity}x
                        {l.volumeCbm != null && <span className="text-gray-500 dark:text-gray-400 ml-1">{l.volumeCbm} CBM</span>}
                        {l.weightKg != null && <span className="text-gray-500 dark:text-gray-400 ml-1">{l.weightKg} KG</span>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        ...
                        {l.quoteCount > 0 && (
                          <span className="ml-1 px-2 py-0.5 rounded-full text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <button onClick={() => { setEditingLoad(l); setView('edit'); }}
                            className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded" title="Edit">
                            <Edit2 className="w-4 h-4" />
                          </button>
                          {(l.status === 'Open') && (
                            <button onClick={() => handleMarkQuoting(l)}
                              className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 rounded" title="Mark as Quoting / Post to Suppliers">
                              <Send className="w-4 h-4" />
                            </button>
                          )}
                          <button onClick={() => setAddQuoteForLoad(addQuoteForLoad?.id === l.id ? null : l)}
                            className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded" title="Add Quote">
                            <Plus className="w-4 h-4" />
                          </button>
                          {l.quoteCount > 0 && (
                            <button onClick={() => handleExpand(l)}
                              className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded" title="View Offers">
                              {expandedId === l.id ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                            </button>
                          )}
                          <button onClick={() => handleDelete(l.id)}
                            className="p-1.5 text-gray-500 dark:text-gray-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded" title="Delete">
                    ...
                    {expandedId === l.id && (
                      <tr>
                        <td colSpan={10} className="px-0 py-0">
                          <div className="bg-gray-50 dark:bg-[#262626] p-6 border-t border-gray-200 dark:border-gray-700">
                            <h4 className="text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-2">
                            ...
                            {quotes.length === 0 ? (
                              ...
                              <div className="space-y-3">
                                {quotes.map(q => (
                                  <div key={q.id} className={`bg-white dark:bg-[#1E1E1E] rounded-lg border p-4 ${q.status === 'Accepted' ? 'border-green-400' : q.status === 'Declined' ? 'border-gray-300 dark:border-gray-600 opacity-60' : 'border-gray-200 dark:border-gray-700'}`}>
                                    <div className="grid grid-cols-6 gap-4 items-center">
                                      <div>
                                        <div className="flex items-center gap-1 text-sm text-gray-900 dark:text-gray-100"><Building2 className="w-4 h-4 text-gray-400" />{q.supplierName}</div>
                                        <div className="text-xs text-gray-500 dark:text-gray-400">Supplier</div>
                                      </div>
                                      <div>
                                        <div className="flex items-center gap-1 text-sm text-gray-900 dark:text-gray-100"><Truck className="w-4 h-4 text-gray-400" />{q.carrierName || '—'}</div>
                                        <div className="text-xs text-gray-500 dark:text-gray-400">Carrier</div>
                                      </div>
                                      <div>
                                        <div className="flex items-center gap-1 text-sm text-green-700 dark:text-green-300"><DollarSign className="w-4 h-4" />{q.currency} {(q.totalRate ?? q.offeredRate ?? 0).toLocaleString()}</div>
                                      ...
                                      <div>
                                        <div className="flex items-center gap-1 text-sm text-gray-900 dark:text-gray-100"><Clock className="w-4 h-4 text-gray-400" />{q.transitDays != null ? `${q.transitDays} days` : '—'}</div>
                                        <div className="text-xs text-gray-500 dark:text-gray-400">Transit</div>
                                      </div>
                                      <div className="flex items-center gap-2 justify-end">
                                        {q.status === 'Accepted' ? (
                                          <span className="px-3 py-1 text-sm bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded-full">Accepted</span>
                                        ) : q.status === 'Declined' ? (
                                          <span className="px-3 py-1 text-sm bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-200 rounded-full">Declined</span>
                                        ) : (
                                          <>
                                    ...
                                    {q.remarks && <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">{q.remarks}</p>}
                                    {q.declineReason && <p className="mt-1 text-xs text-red-600 dark:text-red-300">Declined: {q.declineReason}</p>}
                                  </div>
                                ))}
                              </div>
                            )}
